...
impl PohRecorder {
    ...
    fn report_metrics(&mut self, bank_slot: Slot) {
        if self.last_metric.elapsed().as_millis() > 1000 {
            ...
            // reset metrics for the new PoH Recorder after privious tick
    ...
    pub fn record(
        ...
        assert!(!transactions.is_empty(), "No transactions provided"); // supports only entry with transactions ... or just provided transactions not within an entry

        let ((), report_metrics_us) = measure_us!(self.report_metrics(bank_slot)); // measures the time it takes to send metrics
        self.report_metrics_us += report_metrics_us; // adds time used to send metrics
